...
public class NestedValueTests
{
    ...
    List<(char Indent, int Step, char Delimiter, char QuoteLiteral)> tests =
        [
        (' ', 2, '=','\''),
        (' ', 2, ':','\''),
        ("\u2192"[0], 1, '-','\'')
    ...
    public void VerifyParserMeta()
    {
        ...
        Assert.Equal(':', parser.Delimiter);
        Assert.Equal('\'', parser.QuoteLiteral);
    }
    ...
    public void TestMissDelimited()
    {
        ...
        foreach (var scenario in tests)
        {
            string ccl =
        $"""
		MissDelimitedTest {scenario.Delimiter}
		{TestHelpers.IndentLevel(scenario.Indent, 1, "", scenario.Step)}	Key {scenario.Delimiter} Lorem ipsum dolor sit amet, consectetur adipiscing elit, 
		sed do eiusmod tempor incididunt ut labore et dolore- magna aliqua. 
			Ut enim ad minim: veniam, quis nostrud exercitation ullamco laboris nisi ut 
		      aliquip ex ea commodo consequat= Duis aute irure dolor in reprehenderit in 
				voluptate velit esse cillum dolore eu-fugiat nulla pariatur. Excepteur sint 
